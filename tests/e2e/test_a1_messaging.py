...
- Person creation places the send wrapper script
...
class TestA1MessagingPrompt:
    ...
    def test_a1_prompt_has_bash_send_not_send_message_tool(self, data_dir, make_person):
        """A1 system prompt should reference bash send, not send_message tool."""
        person_dir = make_person("alice")
        # Create a second person so messaging section is populated
        make_person("bob", supervisor="alice")
    ...
    def test_a2_prompt_has_send_message_tool(self, data_dir, make_person):
    ...
    def test_a1_and_a2_both_list_other_persons(self, data_dir, make_person):
...
class TestSendScriptPlacement:
    """Verify that person creation places the send script."""

    def test_create_blank_places_send_script(self, tmp_path):
        """create_blank should place the send script in new person dirs."""
        from core.person_factory import create_blank

        persons_dir = tmp_path / "persons"
        persons_dir.mkdir()

        person_dir = create_blank(persons_dir, "testperson")
        send_script = person_dir / "send"

        assert send_script.exists(), "send script should be placed in person dir"
