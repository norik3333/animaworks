...
from .schemas import ConversationTurn, MemoryBase, MemoryFile, Scenario, ScenarioTypeConfig, SizeConfig
...
class DatasetGenerator:
    ...
    def __init__(self, output_dir: Path, use_llm: bool = True, model: str = "anthropic/claude-sonnet-4-6"):
        """
    ...
    def generate_memory_base(
        self, domain: Literal["business", "tech_support", "education"], size: Literal["small", "medium", "large"]
    ) -> MemoryBase:
        """
        ...
        knowledge_files = self._generate_knowledge_files(
            domain=domain, count=size_config.knowledge_count, output_dir=knowledge_dir
        )

        episode_files = self._generate_episode_files(
            domain=domain, count=size_config.episode_count, output_dir=episodes_dir
        )

        skill_files = self._generate_skill_files(domain=domain, count=size_config.skill_count, output_dir=skills_dir)
    ...
    def _generate_knowledge_files(self, domain: str, count: int, output_dir: Path) -> list[MemoryFile]:
        ...
        for i in range(count):
            ...
            files.append(
                MemoryFile(path=file_path, content=content, tokens=tokens, metadata={"topic": topic, "index": i})
            )

        return files

    def generate_knowledge_file(self, domain: str, topic: str, index: int = 0) -> str:
        """
    ...
    def _generate_knowledge_with_llm(self, domain: str, topic: str, index: int) -> str:
...
Write in Japanese if the domain is business, otherwise use English."""
        ...
        try:
            response = self.litellm.completion(
                model=self.model, messages=[{"role": "user", "content": prompt}], temperature=0.8, max_tokens=1000
            )
    ...
    def _generate_knowledge_template(self, domain: str, topic: str, index: int) -> str:
...
"""
    ...
    def _generate_episode_files(self, domain: str, count: int, output_dir: Path) -> list[MemoryFile]:
        ...
        for i in range(count):
            ...
            files.append(
                MemoryFile(path=file_path, content=content, tokens=tokens, metadata={"date": date_str, "index": i})
            )

        return files

    def generate_episode_file(self, domain: str, date: str, index: int = 0) -> str:
        """
    ...
    def _generate_episode_with_llm(self, domain: str, date: str, index: int) -> str:
...
Write in Japanese if the domain is business, otherwise use English."""
        ...
        try:
            response = self.litellm.completion(
                model=self.model, messages=[{"role": "user", "content": prompt}], temperature=0.8, max_tokens=1500
            )
            content = response.choices[0].message.content
    ...
    def _generate_episode_template(self, domain: str, date: str, index: int) -> str:
...
"""
    ...
    def _generate_skill_files(self, domain: str, count: int, output_dir: Path) -> list[MemoryFile]:
        ...
        for i in range(count):
            ...
            files.append(
                MemoryFile(path=file_path, content=content, tokens=tokens, metadata={"skill": skill_name, "index": i})
            )

        return files

    def generate_skill_file(self, domain: str, skill_name: str, index: int = 0) -> str:
        """
    ...
    def _generate_skill_with_llm(self, domain: str, skill_name: str, index: int) -> str:
...
Write in Japanese if the domain is business, otherwise use English."""
    ...
    def _generate_skill_template(self, domain: str, skill_name: str, index: int) -> str:
...
"""
    ...
    def generate_scenarios(
        self, domain: Literal["business", "tech_support", "education"], memory_base: MemoryBase, total_count: int = 50
    ) -> list[Scenario]:
        """
    ...
    def _select_relevant_memories(self, memory_base: MemoryBase, scenario_type: str) -> list[MemoryFile]:
        ...
        if scenario_type == "factual":
            ...
            files = random.sample(
                memory_base.knowledge_files, min(2, len(memory_base.knowledge_files))
    ...
    def _generate_factual_turns(self, domain: str, relevant_files: list[MemoryFile]) -> list[ConversationTurn]:
        ...
        for i in range(5):
            ...
            turns.append(ConversationTurn(message=message, relevant_memories=relevant_paths))

        return turns

    def _generate_episodic_turns(self, domain: str, relevant_files: list[MemoryFile]) -> list[ConversationTurn]:
    ...
    def _generate_multihop_turns(self, domain: str, relevant_files: list[MemoryFile]) -> list[ConversationTurn]:
    ...
    def _generate_long_conversation_turns(
        self, domain: str, relevant_files: list[MemoryFile]
    ...
    def save_scenarios(self, scenarios: list[Scenario], output_dir: Path) -> list[Path]:
        """
