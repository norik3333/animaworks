...
| `speed` | 話速（1.0 = 標準） |
| `pitch` | ピッチ（0.0 = 標準） |
| `extra` | プロバイダ固有の追加パラメータ（任意）。ElevenLabs では `model_id` でモデルを上書き可能 |
...
| Style-BERT-VITS2 | `model_id:speaker_id` または `model_id:speaker_id:style`。例: `0:0` | `curl http://localhost:5000/models/info` で一覧取得 |
...
- **local_trust モード**: 認証不要
- **trust_localhost 有効かつ localhost 接続**: ループバックアドレスからの接続で認証不要（CSRF検証あり）
- **上記以外**: `session_token` クッキーで検証。無効な場合は 4001 Unauthorized で閉じる

WebSocket は HTTP の Upgrade 時にクッキーを送信するため、ログイン済みブラウザからは自動で認証される。

### 接続制限

- **1 Anima = 1 アクティブセッション**: 同一 Anima への新規接続で既存セッションは置換される（既存側は 4000 "Replaced by new session" で閉じる）
- **不正な name**: `name` に `/` や `..` が含まれる、空、`.` 始まりの場合は 4000 "Invalid anima name" で閉じる
...
| `{"type": "config", "vad_mode": "ptt"}` または `{"type": "config", "vad_mode": "vad"}` | JSON | オプション。モード切替通知（サーバーは現在処理しない） |

### サーバー → クライアント

| タイプ | 形式 | 説明 |
|--------|------|------|
| `{"type": "status", "state": "loading"}` | JSON | セッション初期化中（STT読み込み） |
| `{"type": "status", "state": "ready"}` | JSON | セッション準備完了 |
| `{"type": "transcript", "text": "..."}` | JSON | STT結果テキスト |
| `{"type": "response_start"}` | JSON | Anima応答ストリーム開始 |
| `{"type": "response_text", "text": "...", "done": false}` | JSON | Anima応答テキスト（チャンク） |
| `{"type": "response_done", "emotion": "..."}` | JSON | Anima応答完了（emotion メタデータ付き） |
| `{"type": "thinking_status", "thinking": true/false}` | JSON | 拡張思考（thinking）開始/終了 |
| `{"type": "thinking_delta", "text": "..."}` | JSON | 拡張思考のテキストチャンク |
| TTS音声データ | binary | TTS音声バイナリ |
| `{"type": "tts_start"}` | JSON | TTS音声送信開始 |
| `{"type": "tts_error", "message": "..."}` | JSON | TTS合成失敗時（tts_done の直前に送信） |
...
接続後、サーバーから `status: loading` → `status: ready` が送られ、準備完了後に音声入力を開始できる。
...
- TTSプロバイダが起動しているか確認
  ...
  - SBV2: `curl http://localhost:5000/models/info` でレスポンスがあるか
  - ElevenLabs: `ELEVENLABS_API_KEY` 環境変数が設定されているか
- サーバーログに `TTS unavailable` エラーがないか確認
- クライアントで `tts_error` メッセージが届いていないか確認（TTS合成失敗時）
...
2. **最小発話フィルタ**: 0.35秒未満の音声、無音（RMS閾値以下）は破棄
3. **STT実行**: `speech_end` 受信時にバッファを一括転写
4. **音声モード付加**: メッセージ末尾に `[voice-mode: 音声会話です。話し言葉で200文字以内で簡潔に...]` を付加して Anima に渡す
5. **チャット連携**: ProcessSupervisor IPC経由で既存チャットパイプラインにテキスト送信
6. **TTS前サニタイズ**: Markdown記法（見出し・太字・リスト・コードブロック等）を除去してからTTSに渡す
7. **ストリーミング応答**: Anima応答テキストを文単位で分割（句読点: 。！？、改行）
8. **文単位TTS**: 分割された文ごとにTTSを実行してFirst-byte latencyを最小化
9. **TTSヘルスチェック**: 初回呼び出し時にプロバイダの可用性を確認。成功時のみキャッシュ、失敗時は次回再チェック。連続3回のTTS合成失敗でキャッシュを無効化
10. **並行処理ガード**: 同時に複数の `speech_end` が処理されるのを防止

IPCストリームのタイムアウトはデフォルト60秒。`config.json` の `server.ipc_stream_timeout` で上書き可能。

### 文分割（StreamingSentenceSplitter）

日本語テキストのストリーミング文分割（`core/voice/sentence_splitter.py`）:
- 句読点（。！？!?）の直後で即座に分割
- 改行でも分割
- バッファリングして不完全な文を保持
