...
チャネルDはマッチしたスキル/手続きの**名前のみ**を返却する（段階開示、最大5件）。全文はエージェントが`skill`ツールで必要時に読み込む。チャネルBは統一アクティビティログ（`activity_log/`）と共有チャネルから直近の活動を取得する。heartbeat/cronではチャネルDはスキップされる（誤マッチ防止）。
