...
クレデンシャルは次の優先順位で解決される: `config.json` → `shared/credentials.json` → 環境変数。
...
4. それ以外: `anima_mapping` でチャンネルID → Anima名を解決
5. `Messenger.receive_external()` が `~/.animaworks/shared/inbox/{anima_name}/{msg_id}.json` にメッセージ配置
6. Animaが次のrunサイクル（heartbeat/cron/手動）でinboxを処理

## 関連ファイル

| ファイル | 役割 |
|----------|------|
| `server/slack_socket.py` | SlackSocketModeManager実装（Socket Mode） |
| `server/app.py:141-150, 245-246` | lifespan内での起動/停止 |
| `server/routes/webhooks.py:64-127` | Webhook方式のエンドポイント（`/api/webhooks/slack/events`、mode=webhookの場合） |
| `core/messenger.py` | `receive_external()` — inbox配置 |
| `core/notification/reply_routing.py` | call_human スレッド返信のAnimaルーティング |
| `core/config/models.py:177-192` | `ExternalMessagingChannelConfig` / `ExternalMessagingConfig` モデル |
...
- サーバーログで `"No anima mapping for channel"`（Socket Mode）または `"No anima mapping for Slack channel"`（Webhook）が出ていないか確認

### Webhook モードで署名エラー（400 Invalid signature）

- `SLACK_SIGNING_SECRET` が設定されているか確認
